...
(defn- how-many-mines [depth] ;; CREATES A VECTOR OF THE SURROUNDING SQUARES' COORDINATES WITH A PERMUTATION FUNCTION
  (let [mines (vec (range (- depth) (inc depth)))] ;; RANGE OF SQUARES TO LOOK THRU BASED ON PARAMETER 'depth'
    (mapv vec (mapcat (fn [x] (map (fn [y] (concat [x] [y])) mines)) mines)))) ;; RETURNS A VECTOR OF ALL POSSIBLE COORDINATES (SHORTENED PERMUTATION)
  ...
  (count ;; COUNTS HOW MANY TRUES ARE PRESENT WITHIN A CONCATENATED VECTOR
    (filter identity ;; REMOVES FALSE BY FILTERING OUT NON-TRUE AND NIL
      (map 
        (fn [x] 
          (try
            (get-in (@app-state :bool-grid) [(first x) (second x)])
            (catch js/Error e false) ;; INDEX OUT OF BOUNDS ERROR CHECKING WITH REACT.JS
            (finally (reset! app-state (assoc-in @app-state [:visual-grid i j] "X")))))
      (coordinates i j)))))
